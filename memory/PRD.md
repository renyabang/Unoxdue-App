...
## 📣 Integrazione TELEGRAM + mockup SPONSOR B2B (30 giugno 2026)
- **Anti-Short DEPLOYATO in produzione** (l'utente ha lanciato il comando sul VPS: git pull dd0ba77 + rebuild backend). Short ≤180s ora esclusi dal sync.
- **Micuccio**: ruolo aggiornato a "Analista e Specialista Campionati Minori" — applicato DIRETTO in produzione via API (`POST /api/admin/team`, dato non codice, niente deploy), verificato live su /team/.
- **Telegram (backend + admin) — COMPLETATO**:
  - Nuovo modulo `backend/telegram_bot.py`: Bot API via httpx (outbound, niente webhook). Token+canale salvati in `db.settings.telegram` (mai mostrati in chiaro: API ritorna `token_masked`). Funzioni: getMe/getChat/getChatMember (test connessione + verifica permesso "Pubblica messaggi"), sendMessage/sendPhoto (fallback a testo se l'immagine è rifiutata)/sendPoll. Rendering modelli con escaping HTML. Log invii in `db.telegram_messages` + automation_logs.
  - 4 tipi messaggio (modelli EDITABILI da admin): 🎙️ nuova puntata (con copertina/thumbnail), 🎯 pronostico/schedina (con grafica brandizzata: pick graphics PNG → cover square), 🔴 avviso live Twitch, 🗳️ sondaggio nativo.
  - Endpoint `/api/admin/telegram/{config(GET/PUT), test, send-test, preview, publish/episode, publish/prediction, publish/live, publish/poll, messages}`.
  - Auto-pubblicazione (OFF di default, con dedup): `maybe_autopost_episode` (flag `telegram_posted` su episode; hook in `upsert_episode` + `transcripts/seo/publish`), `maybe_autopost_prediction` (flag `telegram_posted_picks` su prediction; hook in `add_pick` + `predictions/ai/publish`). Mai bloccanti (try/except).
  - Admin: nuova pagina `frontend/src/admin/Telegram.jsx` + voce menu "Telegram" + route `/admin/telegram` + metodi `api.js`. Sezioni: configurazione (token mascherato, canale, twitch), test+messaggio prova, automazioni (3 toggle), editor 3 modelli, pubblicazione manuale (puntata/pronostico/live/sondaggio con anteprima), log ultimi invii.
  - Canale utente: bot "Unoxdue Bot" già admin del canale (ok). Verificato: preview rendering episodio (testo+thumbnail), publish/test senza token ritornano ok:false puliti (no 500). Pannello renderizzato (screenshot). **INVIO REALE A TELEGRAM da verificare dall'utente** (serve il Bot Token che l'utente inserisce nel campo sicuro dell'admin).
...
## ✉️ NEWSLETTER via Resend (1 luglio 2026) — scaffolding COMPLETO e testato; invio da attivare con API key
- **Playbook**: Resend SDK (`resend==2.32.2`, in requirements). Chiave da env `RESEND_API_KEY` (aggiunta vuota in backend/.env). Single opt-in (niente conferma), link disiscrizione obbligatorio.
- **Backend `newsletter.py`**: config in `db.settings.newsletter` (sender_email default newsletter@unoxdue.net, from_name, owner_email, enabled, auto_episode, templates episode/live). Iscritti in `db.subscribers` (single opt-in, unsub_token). Funzioni: subscribe/unsubscribe, `_wrap` (email HTML brandizzata inline, table layout, footer con unsubscribe), render_episode/render_live, `send_test`, `send_campaign` (Resend Batch a blocchi di 100, unsub link personalizzato), `notify_owner`, `resolve`, `maybe_autosend_episode` (dedup flag `newsletter_sent`).
- **Endpoint**: pubblici `POST /api/newsletter/subscribe`, `GET /api/newsletter/unsubscribe?token=` (pagina HTML noindex). Admin `GET/PUT /admin/newsletter/config`, `GET /admin/newsletter/subscribers`, `GET .../subscribers/export` (CSV), `DELETE .../subscribers/{id}`, `POST /admin/newsletter/preview|test|send`.
- **Auto hook**: `nl.maybe_autosend_episode` in `upsert_episode` + `transcripts/seo/publish` (accanto a Telegram). **Notifica sponsor**: `/api/sponsor/lead` ora chiama `nl.notify_owner` (email al proprietario con i dati del lead) — si attiva con Resend.
- **Frontend**: form iscrizione nel **footer** (base.html, tutte le pagine, JS fetch, messaggi inline). Admin `Newsletter.jsx` (menu "Newsletter", route `/admin/newsletter`): config + warning se API key assente, editor modelli (episode/live), invio (episode/live/generic con anteprima in iframe + test a sé + invio a tutti), lista iscritti + export CSV + elimina, stato ultimo invio. Metodi in `api.js`.
- **Verificato**: subscribe ok/invalid, unsubscribe 200, admin config (apikey_set False, sender ok), preview episodio (subject con {title} + HTML), send senza key → "RESEND_API_KEY non configurata" pulito, form footer submit (browser) → "Iscrizione confermata", pannello admin screenshot OK. Iscritti di test ripuliti.
- **DA FARE (utente)**: creare account Resend (gratis), verificare dominio unoxdue.net (record SPF/DKIM su SiteGround DNS), creare API Key `re_...`. Poi: inserire `RESEND_API_KEY` in backend/.env (preview) e in produzione (/root/unoxdue/.env), riavviare/rebuild. Solo allora l'invio reale funziona (in test mode Resend invia solo all'email registrata finché il dominio non è verificato).
